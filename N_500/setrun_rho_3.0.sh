...
do
    ...
    do
        ...
        do
            ...
            sed -i "s/Box_VAL/5.503212/g"          run
            sed -i "s/A_VAL/$Alpha/g"           run
            sed -i "s/R_VAL/$Rcut/g"            run

            if (( $(awk 'BEGIN {print ('"$L"' > 0.9)}') )); then
                sed -i "s/D_VAL/0.01/g" run
            elif (( $(awk 'BEGIN {print ('"$L"' > 0.7)}') )); then
                sed -i "s/D_VAL/0.05/g" run
            elif (( $(awk 'BEGIN {print ('"$L"' > 0.5)}') )); then
                sed -i "s/D_VAL/0.1/g" run
            elif (( $(awk 'BEGIN {print ('"$L"' > 0.3)}') )); then
                sed -i "s/D_VAL/0.5/g" run
            else
